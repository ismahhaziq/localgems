...
class _ImageAndCategorySelectionState extends State<ImageAndCategorySelection> {
  ...
  void _saveDataAndNavigateBack() async {
    ...
    try {
      ...
      _showErrorDialog('Failed to upload place. Please try again.');
    } finally {
      setState(() {
        _uploadingImages = false;
      });
...
void _showSuccessDialog() {
    showDialog(
      context: context,
      barrierDismissible: false, // Makes the dialog non-dismissible by touch
      builder: (BuildContext context) {
        return WillPopScope(
          onWillPop: () async =>
              false, // Prevents back gesture from closing the dialog
          child: AlertDialog(
            title: Text('Place Submitted'),
            content: Text(
                'Your place has been submitted and is pending admin approval.'),
            actions: [
              TextButton(
                onPressed: () {
      ...
      // Ensure navigation if dialog is dismissed
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(builder: (context) => NavBar()),
         (route) => false,
