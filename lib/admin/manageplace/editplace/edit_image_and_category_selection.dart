...
class _ImageAndCategorySelectionState extends State<ImageAndCategorySelection> {
  ...
  void initState() {
    super.initState();
    _selectedKeywords = List.from(widget.initialSelectedKeywords);
  ...
  void _removeInitialImage(int index) {
    setState(() {
  ...
  void _showFullImageDialogFromURL(String imageUrl) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return Dialog(
  ...
  Widget _buildPhotoPreviewFromURL(String imageUrl, int index) {
    return Stack(
      children: [
        GestureDetector(
          onTap: () {
            _showFullImageDialogFromURL(imageUrl);
          },
          child: Container(
            margin: EdgeInsets.all(4),
            decoration: BoxDecoration(
  ...
  void _saveDataAndNavigateBack() async {
    if (_images.isEmpty && _initialImageURLs.isEmpty ||
    ...
    try {
      List<String> imageURLs = List.from(_initialImageURLs);
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    if (_images.length + _initialImageURLs.length > 6)
                      Padding(
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        child: Wrap(
          ...
          runSpacing: 8.0,
          children: [
            ..._initialImageURLs.map((imageUrl) {
              return _buildPhotoPreviewFromURL(
                  imageUrl, _initialImageURLs.indexOf(imageUrl));
            }).toList(),
            ..._images.map((photo) {
              return _buildPhotoPreview(photo);
            }).toList(),
          ],
        ),
      ),
