...
class _ComplaintAdminDetailsPageState extends State<ComplaintAdminDetailsPage> {
  ...
  void initState() {
    ...
    if (widget.complaintData.containsKey('adminComment')) {
      _commentController.text = widget.complaintData['adminComment'];
    }
  }
  ...
  Future<void> _updateComplaintStatus(
    ...
    bool confirmAction = await _showConfirmation(context, status);
    if (confirmAction) {
  ...
  Future<bool> _showConfirmation(BuildContext context, String status) async {
    String actionText;
    if (status == 'reviewed') {
              ...
              Row(
                ...
                children: [
                  _buildActionButton(
                      'Mark as Reviewed',
                      Colors.blue,
                      (context) => _updateComplaintStatus(context, 'reviewed'),
                      context),
                  _buildActionButton(
                      'Resolve Complaint',
                      Colors.green,
                      (context) => _updateComplaintStatus(context, 'resolved'),
                      context),
                ],
              ),
              SizedBox(height: 16),
              Row(
                ...
                children: [
                  _buildActionButton(
                      'Suspend Place',
                      Colors.red,
                      (context) => _updateComplaintStatus(context, 'suspended'),
                      context),
                  _buildActionButton(
                      'Dismiss Complaint',
                      Colors.grey,
                      (context) => _updateComplaintStatus(context, 'dismissed'),
                      context),
                ],
              ),
            ],
